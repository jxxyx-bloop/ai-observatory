...
|---|---|---|
| **01 読む** | **02 測る** | **03 動く** |
| エージェントはセッションごとに記録を残しています。その場にあるファイルをそのまま解析します。 | トークン、キャッシュ、時間帯、コスト。実行された時点で有効だった料金で計算します。 | 優先順位つきの変更リスト。根拠と、月あたりの価値が必ず添えられます。 |
...
- 解析の入口で遮断しています。だから漏えいは方針の問題ではなく、バグです。
