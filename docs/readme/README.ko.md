...
`토큰 소모 없음` · `계정 불필요` · `데이터는 기기 밖으로 나가지 않음`
...
|---|---|---|
| **01 읽기** | **02 측정** | **03 실행** |
| 에이전트는 세션마다 기록을 남깁니다. 그 파일을 있는 자리에서 그대로 해석합니다. | 토큰, 캐시, 시간대, 비용 — 작업이 실제로 실행된 시점의 요율로 계산합니다. | 우선순위가 매겨진 변경 목록. 근거와 월 단위 가치가 함께 붙습니다. |
...
- 파일을 파싱하는 지점에서 차단합니다. 그래서 유출은 정책 문제가 아니라 버그입니다.
